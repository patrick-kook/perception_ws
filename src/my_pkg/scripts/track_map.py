...
# 이미지 경로 및 파라미터 설정
image_path = "/home/patrick/Downloads/track_img_2.jpeg"
SCALE_FACTOR = 0.0101
SAMPLE_DISTANCE = 1
...
def publish_points(track_points, red_trajectory_points, green_trajectory_points, yellow_trajectory_points):
    ...
    traj_pub = rospy.Publisher("/trajectory", Marker, queue_size=10)  # /trajectory 토픽에 빨강, 초록, 노랑 포인트 퍼블리시
    ...
    # Trajectory Marker 객체 (빨간색, 초록색, 노란색 경로)
    traj_marker = Marker()
    ...
    while not rospy.is_shutdown():
        ...
        traj_pub.publish(traj_marker)    # 빨강, 초록, 노랑 경로 퍼블리시
        rate.sleep()
...
if __name__ == "__main__":
    ...
    traj_center_x, traj_center_y = calculate_center(red_trajectory_points + green_trajectory_points + yellow_trajectory_points)
    ...
    # 포인트 퍼블리시
    publish_points(track_points, red_trajectory_points, green_trajectory_points, yellow_trajectory_points)

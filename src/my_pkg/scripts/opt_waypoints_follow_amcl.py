...
class PurePursuit:
    ...
    def __init__(self):
        ...
        # 웨이포인트와 오도메트리 구독자
        rospy.Subscriber("/global_path/optimal_trajectory", Marker, self.waypoints_callback)
    ...
    def obs_waypoints_callback(self, msg):
        self.obs_waypoints = np.array(
            [[pose.x, pose.y] for pose in msg.points]
        )
        self.colors = np.array(
            [[color.r, color.g, color.b, color.a] for color in msg.colors]
        )
    ...
    def get_target_point(self, lookahead_distance):
        current_waypoints = (
            self.obs_waypoints if self.obstacle_detected else self.waypoints
            or self.standard_wapoints if self.obstacle_detected else self.waypoints
        )
